...
describe("PDS calls work", () => {
    ...
    test('we should be able to get an adequate response when validating a valid NHS number', async () => {
        ...
        mock.onPost('https://msg.opentest.hscic.gov.uk/smsp/pds', pds_client.generateGetNHSNumberRequest()).reply(200, given.generateValidPdsGetNHSNumberResponseFor(nhsNoToVerify));
    ...
    test('we should be able to get an adequate response when validating a invalid NHS number', async () => {
        const nhsNoToVerify = "9999345201";
        mock.onPost('https://msg.opentest.hscic.gov.uk/smsp/pds', pds_client.generateGetNHSNumberRequest()).reply(200, given.generateInvalidPdsGetNHSNumberResponse());
