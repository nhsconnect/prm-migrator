locals {
  function_name = "ehr-translate-send-${var.environment}"
}

resource "aws_s3_bucket_object" "lambda" {
  bucket = "prm-${data.aws_caller_identity.current.account_id}-lambda-images"
  key    = "send_lambda-${var.environment}.zip"
...
resource "aws_lambda_function" "lambda" {
  ...
  description      = "EHR translate send EHR API handler for ${var.environment}"
  memory_size      = 128
  timeout          = 20
  runtime          = "nodejs8.10"
  ...
  tags {
    Name          = "ehr-translate-send-${var.environment}"
